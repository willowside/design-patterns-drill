...
func (app *application) render(w http.ResponseWriter, t string, td *templateData) {
	...
	// template cache, stores a map info of ready to render template
	// if we are using the template cache, try to get the template from our map, stored om the receiver
	// 1. determine whether or not we're using the template
	// 2. need somewhere to store the template cache, which is gonna be a map
